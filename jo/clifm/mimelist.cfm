                  ##########################################################
                  #   Configuration file for Lira CliFM's resource opener  #
                  ##########################################################

# created on first invocation of  clifm, so
#  nvim -O /usr/share/clifm/mimelist.cfm $ARCHBUILDS/jo/clifm/keybindings.cfm -c 'windo difft'
#  nvim -O $ARCHBUILDS/jo/clifm/mimelist.cfm ~/.config/clifm/profiles/default/mimelist.cfm -c 'windo difft'

# Commented and blank lines are omitted.

# The below settings cover the most common filetypes.
# It is recommended to edit this file placing your prefered applications at the beginning of the apps list to speed up the opening process.

# The file is read top to bottom and left to right; the first existent application found will be used.

# Applications defined here are NOT desktop files, but commands (arguments could be used as well). Write you own handmade scripts to open specific files if necessary. Ex: X:^text/.*:~/scripts/my_cool_script.sh.
...
# Use the %f placeholder to specify the position of the file name to be opened in the command. Example: 'mpv %f --terminal=no' -> 'mpv FILE --terminal=no'.
# If %f is not specified, the file name will be added to the end of the command. Ex: 'mpv --terminal=no' -> 'mpv --terminal=no FILE'.

# Running the opening application in the background:
# For GUI applications: APP %f &
# For terminal applications: TERM -e APP %f &
# Replace 'TERM' and 'APP' by the corresponding values. The -e option might vary depending on the terminal emulator used (TERM).

# To silence STDERR and/or STDOUT use !E and !O respectivelly (they could be used together). Examples:
# Silence STDERR only and run in the foreground: mpv %f !E
# Silence both (STDERR and STDOUT) and run in the background: mpv %f !EO &
# or mpv %f !E !O &

# Environment variables could be used as well. Example: X:text/plain=$TERM -e $EDITOR %f &;$VISUAL;nano;vi.
